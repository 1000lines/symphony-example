...
| `symphony-client-wakeups.yml`        | CT-C reusable `symphony-linear-wakeups.yml`; actual target/check/run identities, Linear token only.                                                                                                                      |
| `symphony-client-ci.yml`             | Optional CT-C `symphony-client-commands.yml` caller, no review secrets. Omit when application CI already suffices.                                                                                                       |

CT-C's reviewed code exists on main through
[#45](https://github.com/1000lines/symphony-example/pull/45), merge
`fd383f5760a2ba62ea6f6295bd6dd21cc0cb9e9e`. Integration remains CT-L-owned under
the inventory. CT-R's [#43](https://github.com/1000lines/symphony-example/pull/43)
...
verify explicit secrets at every hop, close native/Docker/remote integration,
register the observed render check and reconcile this list before CT-U/O/V.
