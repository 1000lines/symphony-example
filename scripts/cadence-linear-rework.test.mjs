...
test("Cadence human-review workflows do not request broad teams", () => {
  ...
  for (const workflowPath of workflowPaths) {
    ...
    assert.match(
      workflow,
      /uses: 1000lines\/symphony-client-workflows\/\.github\/workflows\/cadence-(?:linear-rework|ai-review-trigger)\.yml@main\s*$/m
    );
  }
});
...
test("Cadence review-request trigger stays wired for Cadence self requests", () => {
  ...
  assert.match(workflow, /uses: 1000lines\/symphony-client-workflows\/\.github\/workflows\/cadence-ai-review-trigger\.yml@main\s*$/m);
